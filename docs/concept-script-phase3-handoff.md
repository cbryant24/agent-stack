...
- **Refinement pass already landed (2026-06-07)**, recorded in `docs/v2-refinements-concept-script.md` under "Addressed in a refinement pass." Two `shape` defects found in real-input testing were *fixed*, not deferred:
...
**End condition.** All Phase-3-scoped items either landed or remain in `docs/v2-refinements-concept-script.md` with documented reasoning for the defer; that file stays current; all documentation reflects the post-Phase-3 state; a handoff is produced for the next agent, tool, or application.
