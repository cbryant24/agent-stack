...
**Status: Phase 2 — first build slice shipped.** 14 tests passing. Supersedes the previously-planned "Project Organizer Agent." That agent was scoped to file scaffolding and manifest/status tracking, and was flagged as largely redundant with Cowork — which the user does plan to use for project scaffolding. The remaining file-organization gap is small and stays with Cowork; what the user actually wants in this slot is a different kind of agent, specified below.
...
**Still deferred (not yet built).** Vector-DB diagnostics + per-agent remediation delegation (design settled, below); the other three agents as tools (`voiceover-direction`, `concept-script`, `visual-generation`) — added after the contract is proven on the two seed agents; MCP (both wrapping agents and exposing the orchestrator); additional surfaces (Telegram / voice / web / scheduled); Haiku utility (tool-output compression, long-thread summarization); the per-session hard ceiling (v1 is a soft tally only); and the schema-migration runner/ledger.
...
9. **Orchestrator** — done, Phase 2 first build slice (14 tests passing). The conversational meta-agent over the whole system (LangGraph ReAct loop, SQLite checkpointer, knowledge + repo-access + two seed sub-agents as tools); supersedes the old "Project Organizer" slot, whose file-scaffolding scope stays with Cowork. Remaining first-slice deferrals (diagnostics, the other three agents as tools, MCP, extra surfaces, Haiku utility, the hard ceiling) are tracked in its section above.
