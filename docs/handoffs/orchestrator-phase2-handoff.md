...
- **Model constant:** `MODEL_ORCHESTRATOR` (= `claude-sonnet-4-6`) lives in the orchestrator package (`orchestrator/constants.py`), per the per-package convention (music-curation's `MODEL_GENERATOR`, voiceover-direction's `MODEL_DIRECTOR`). It is not defined in `agent-runtime`.
