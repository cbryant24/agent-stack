...
def test_budget_exhaustion_produces_partial_result():
    # process_video raises BudgetExhaustedError (e.g. cost limit hit mid-pipeline) → partial
    from agent_runtime.exceptions import BudgetExhaustedError as _Exc
    from agent_runtime.models import BudgetConsumption

    tight_budget = BudgetEnvelope(max_cost_usd=0.001)

    async def mock_process(url: str, **kwargs):
        raise _Exc("max_cost_usd", tight_budget, BudgetConsumption())
